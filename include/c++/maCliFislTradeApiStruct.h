...
struct MATRADEAPI CRspFislQryColStkInfoField
{
  ...
  char          szCollatCls[2 + 1];           // 担保类别 字典[COLLAT_CLS]
};
...
struct MATRADEAPI CRspFislCancelOrderField
{
  ...
  long long     llFortuneCode;                // 财富账户(未启用)
};
...
struct MATRADEAPI CReqFislMaxTradeQtyField
{
  ...
  char            szTrdacctLink[20 + 1];      // 关联股东 ETF实物申赎时送（接口预留，暂不实现）
  char            szMeetingSeq[10 + 1];       // 股东大会编码 
  char            szVoteId[10 + 1];           // 议案编号 
  char            chEtfMode;                  // ETF模式 字典ETF_MODE
...
struct MATRADEAPI CReqFislMarginRepayField
{
  ...
  char            szOrderText[1024 + 1];      // 委托扩展 直接还款指定合约格式:关键字1=值1,关键字2=值2，例如?ORDER_TEXT:DEBTLIST=合约日期1|合同序号1;合约日期2|合同序号
};
...
struct MATRADEAPI CRspFislQryExpendableFundField
{
  ...
  char          szLwlmtFund[21 + 1];          // 预留金额 资金下限
};
...
struct MATRADEAPI CRspFislQryFreqTradeLmtParamField
{
  ...
  LONGLONG      llCuacctCode;                    // 资产帐号
  char          szStkbd[2 + 1];                  // 交易板块
  char          szTrdacct[20 + 1];               // 股东账户
  int           iTimeslice;                      // 流速时间片
  int           iTimesliceUplmtNum;              // 流速阈值
  int           iDayUplmtNum;                    // 当日流量阈值
};

//-------------------------------10349043:可用资金查询(次席调用)----------------------------
...
struct MATRADEAPI CRspFislQryMarginContractField
{
  ...
  char          szDebtProfit[21 + 1];         // 负债盈亏 
  char          szRefProfit[21 + 1];          // 理论盈亏 
  char          szContractRate[21 + 1];       // 合约利率 
  char          szUnreplayFund[21 + 1];       // 未还金额 
  LONGLONG      llUnreplayQty;                // 未还数量 
  char          chOverdueFlag;                // 逾期标志       
  char          szCmpdInt[21 + 1];            // 复利金额
  char          szCmpdPrin[21 + 1];           // 复利本金
};
...
struct MATRADEAPI CRspFislQryCreditCustDebtsField
{
  ...
  char            szStkValue[21 + 1];         // 证券市值 
  char            szTotalSlAmt[21 + 1];       // 总融券卖出资金 
  char            szTotalSlInt[21 + 1];       // 融券预计利息
  char            szTotalAssertGy[21 + 1];    // 公允资产
  char            szStkValueGy[21 + 1];       // 公允市值
};
...
struct MATRADEAPI CReqFislQryMarginSharesCashField
{
  ...
  LONGLONG        llCuacctCode;               // 资产账户 
  char            chQueryFlag;                // 查询方向 0:向后取数据 1:向前取数据 其他全部返回
  char            szQueryPos[32 + 1];         // 定位串 入值为记录号 与查询方向配合使用
  int             iQueryNum;                  // 查询行数 
};
...
struct MATRADEAPI CRspFislQryMarginSharesCashField
{
  ...
  char            chCashType;                 // 头寸类型 
  char            szSrkName[64 + 1];          // 证券名称 
  LONGLONG        llRepayQty;                 // 当日还券 
  LONGLONG        llSlUnlockQty;              // 锁券数量 
  LONGLONG        llSlRemainQty;              // 剩余融券数量 
  LONGLONG        llRemainQty;                // 仍需还券数量 
  char            szQryPos[32 + 1];           // 定位串 
};
...
struct MATRADEAPI CReqFislContractExtField
{
  ...
  int           iEndDate;                   // 结束日期
  LONGLONG      llCustCode;                 // 客户代码
  char          szStkbd[2 + 1];             // 市场板块
  char          szOrderId[10 + 1];          // 合同序号
  char          chApplyStatus;              // 申请状态
  LONGLONG      llCuacctCode;               // 资产账户
  int           iIntOrg;                    // 内部机构
  char          szQueryPos[32 + 1];         // 定位串
  int           iQueryNum;                  // 查询行数
  int           iKOpeningDate;              // 最后一条记录开仓日期
  LONGLONG      llKCustCode;                // 最后一条记录客户号
  char          szKStkbd[2 + 1];            // 最后一条记录交易板块
  char          szKOrderId[10 + 1];         // 最后一条记录合约序号
  char          szKOccureDate[40 + 1];      // 最后一条记录发生日期
};
...
struct MATRADEAPI CRspFislContractExtField
{
  ...
  char          chStkex;                    // 交易市场
  char          szQueryPos[32 + 1];         // 定位串
  LONGLONG      llTotalNum;                 // 总记录数    
  char          szOccurTime[32 + 1];        // 申请时间 yyyy-mm-dd hh24miss.nnn
};
...
struct MATRADEAPI CReqFislUserLoginField
{
  ...
  char          chPasswordType;               // 密码类型，'0'-明文，'1'-密文，如果没有设置值默认表示明文
};
...
struct MATRADEAPI CRspFislUserLoginField
{
  ...
  char          szThirdParty[1024 + 1];       // 第三方 
  char          chTrdacctMainFlag;            // 主股东账户标志 ‘0’- 辅股东账户  ‘1’- 主股东账户
  char          chCuacctSyncFlag;             // 单账户多节点标志 ‘0’-非单账户多节点，‘1’-单账户多节点
  char          szFlashIp[64 + 1];            // 软加速地址
};
...
struct MATRADEAPI CRspFislQryStockEffectSummaryField
{
  ...
  int			  iBizCode;					  // 业务代码
  char			  szBizContext[128 + 1];	  // 业务内容
  LONGLONG	      szStkBlnEffect;		      // 股份余额变化
  LONGLONG		  szStkAvlEffect;		      // 股份可用变化
  char			  chReverseStatus;			  // 冲正状态
};
//-------------------------------10306028:日切交易日查询--------------------------
...
struct MATRADEAPI CReqFislFundTransferJzjyField
{
  ...
  LONGLONG        llOpCode;                   // 操作柜员编号 查询集中交易股份需要柜员权限
  char            chIsEffectBln;              // 是否划拨余额 ‘0’:不划拨余额 ‘1’:划拨余额 （默认值’1’）
};
...
struct MATRADEAPI CReqFislContApplyField
{
  ...
  char            szStkbd[2 + 1];             // 交易板块 
  int             iOpeningDate;               // 开仓日期 
  char            chCompIniFlag;              // 启用复利标志 0-不启用 1-启用
};
...
struct MATRADEAPI CReqFislFundTransferNodeField
{
  ...
  char            chTranType;                 // 划拨类型  '0'-划拨可用 '1'-划拨可取
};
...
//-------------------------------10349044:资金划拨(单账户多节点) 订单节点之间划拨[次席调用]------------------
...
struct MATRADEAPI CReqFislQryBlockTradingMktInfoField
{
  ...
  char            szQueryPos[32 + 1];         // 定位串 
  int             iQueryNum;                  // 查询行数 
  char            chMkCls;                    // 行情类别 1：意向行情 2：定向行情
  char            chSide;                     // 买卖方向 
};
...
struct MATRADEAPI CRspFislQryBlockTradingMktInfoField
{
  ...
  char            chExecType;                 // 执行类型 0：申报  1：成交 2：撤单委托
};
...
struct MATRADEAPI CReqFislQryClosingBlockTradingMktInfoField
{
  char            chStkex;                    // 市场 
  char            szStkCode[32 + 1];          // 证券代码 
  char            chStkPhQuoteType;           // 盘后报价类型 ‘1’：收盘价 ‘2’：平均价
...
struct MATRADEAPI CReqFislCombOrderField
{
  ...
  int             iStkBiz;                    // 证券业务 字典[STK_BIZ]
  int             iStkBizAction;              // 业务行为 字典[STK_BIZ_ACTION]
  int             iOrderBsn;                  // 委托批号 送入批号表示补委托处理,为-1或不送表示初始委托
  int             iCuacctSn;                  // 账户序号 
  ...
  LONGLONG        llCustCode;                 // 客户代码 
  char            szClientInfo[256 + 1];      // 终端信息 硬盘号\CPU\机器名等 格式： "PC;IIP=公网IP地址;IPORT=公网IP端口号;LIP=内网IP;MAC=MAC地址;HD=硬盘序列号;PCN=PC终端设备名;CPU=CPU序列号;PI=硬盘分区信息;VOL=系统盘卷标号@扩展信息1;扩展信息2;扩展信息3"
...
struct MATRADEAPI CRspFislCombOrderField
{
  ...
  int             iStkBiz;                    // 证券业务 字典[STK_BIZ]
  int             iCuacctSn;                  // 账户序号 
  int             iOrderRet;                  // 委托结果 0:表示成功 -1:表示失败
...
{
  LONGLONG        llCustCode;                 // 客户代码 客户代码
  LONGLONG        llCuacctCode;               // 资产账户 资产账户
  char            chQueryFlag;                // 查询方向 0:向前取数据 1:向后取数据
  char            szQueryPos[32 + 1];         // 定位串 
  int             iQueryNum;                  // 查询行数 
  char            chChannel;                  // 操作渠道
