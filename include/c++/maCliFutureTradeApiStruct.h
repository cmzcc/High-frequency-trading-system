...
struct MATRADEAPI CReqFutureOrderField
{
  ...
  char     szCombHedgeFlag[5 + 1];//组合投机套保标志 COMB_HEDGE_FLAG      VARCHAR(5) X 8742
...
struct MATRADEAPI CRspFutureOrderInfoField
{
  ...
  char     szActiveTraderId[21 + 1];   //最后修改交易所交易员代码 ACTIVE_TRADER_ID       VARCHAR(21)8727
...
struct MATRADEAPI CRspFutureCustInfoField
{
  ...
  char     szCommmodelId[13 + 1];    //手续费率模板代码 COMMMODEL_ID        VARCHAR(13) 8771
  char     szMarginmodelId[13 + 1];  //保证金率模板代码 MARGINMODEL_ID      VARCHAR(13) 8772
};
...
{
  char     szCuacctCode[16 + 1];     //资产账户 CUACCT_CODE      VARCHAR(16)     Y 8920CTP  客户ID
  char     szCustCode[16 + 1];       //客户代码 CUST_CODE      VARCHAR(16)
  char     szInstrumentId[31 + 1];   //合约代码 INSTRUMENT_IDVARCHAR(31)X 48
};
...
struct MATRADEAPI CRspFuturePosInfoField
{
  ...
  char     chPosiDirection;     //持仓多空方向 POSI_DIRECTION  VARCHAR(1) 8740
  LONGLONG llPositionVolume;    //持仓量   POSITION_VOLUME BIGINT     9351
  LONGLONG llExeAvl;            //可执行量  EXE_AVL         BIGINT     8869
  int      iCloseFrzVolume;     //平仓冻结手数 CLOSE_FRZ_VOLUMESMALLINT   8870
...
struct MATRADEAPI CReqFutureContractInfoField
{
  char     szInstrumentId[31 + 1];//合约代码    INSTRUMENT_ID   VARCHAR(31)X 48
  char     szExchangeId[9 + 1];//交易所代码   EXCHANGE_ID     VARCHAR(9) X 207
  char     szExchangeInstId[31 + 1];//合约在交易所代码 EXCHANGE_INST_IDVARCHAR(31)X 8725
...
struct MATRADEAPI CRspFutureContractInfoField
{
  ...
  char     chMaxMarginSideAlgorithm;    //是否使用大额单边保证金算法 MAX_MARGIN_SIDE_ALGORITHMVARCHAR(1) 8686
};
...
struct MATRADEAPI CRspFutureFundInfoField
{
  ...
  char     szExeDynamicMargin[21 + 1];//执行冻结保证金 EXE_DYNAMIC_MARGIN CMONEY     9410
...
struct MATRADEAPI CReqFutureAvlQtyField
{
  ...
  char     szExchangeId[9 + 1];      //交易所代码 EXCHANGE_ID  VARCHAR(9) Y 207
...
{
  LONGLONG llOrderQty;             //可开仓手数 ORDER_QTY      BIGINT 38  可委托最大数量
  char     szAvailableAmout[21 + 1]; //可用资金 AVAILABLE_AMOUTCMONEY 8611
  LONGLONG llBuyOrderQty;          //可买开手数 BUY_ORDER_QTY  BIGINT 9080
  int      iSellOrderQty;          //可卖开手数 SELL_ORDER_QTY INTEGER9081
...
struct MATRADEAPI CRspFutureAvlPosField
{
  ...
  char     szInstrumentId[31 + 1];//合约代码 INSTRUMENT_ID  VARCHAR(31)48
  char     szAvailableAmout[21 + 1];//可用资金 AVAILABLE_AMOUTCMONEY     8611
  LONGLONG llOrderQty;          //可平仓手数 ORDER_QTY      BIGINT     38  可委托最大数量
  LONGLONG llTodayOrderQty;     //可平今手数 TODAY_ORDER_QTYBIGINT     9081可委托最大数量
...
{
  int      iUerNum;            //客户量 UER_NUMSMALLINT8600
};
...
struct MATRADEAPI CReqFutureFeeField
{
  ...
  char     szExchangeId[9 + 1];//交易所代码 EXCHANGE_ID  VARCHAR(9) X 207 交易所代码
  char     szInstrumentId[31 + 1];//合约代码 INSTRUMENT_IDVARCHAR(31)X 48
};
...
struct MATRADEAPI CRspFutureFeeField
{
  ...
  char     szBuketOpenCommissionByqty[21 + 1];   //投机开平手续费按手数 BUKET_OPEN_COMMISSION_BYQTY     CMONEY     9415
  char     szBuketOpenCommissionByamt[21 + 1];   //投机开平手续费按金额 BUKET_OPEN_COMMISSION_BYAMT     CMONEY     9416
  char     szBuketCloseCommissionByqty[21 + 1];  //投机平今手续费按手数 BUKET_CLOSE_COMMISSION_BYQTY    CMONEY     9417
  char     szBuketCloseCommissionByamt[21 + 1];  //投机平今手续费按金额 BUKET_CLOSE_COMMISSION_BYAMT    CMONEY     9418
  char     szHedgeOpenCommissionByqty[21 + 1];   //保值开平手续费按手数 HEDGE_OPEN_COMMISSION_BYQTY     CMONEY     9419
  char     szHedgeOpenCommissionByamt[21 + 1];   //保值开平手续费按金额 HEDGE_OPEN_COMMISSION_BYAMT     CMONEY     9420
  char     szHedgeCloseCommissionByqty[21 + 1];  //保值平今手续费按手数 HEDGE_CLOSE_COMMISSION_BYQTY    CMONEY     9421
  char     szHedgeCloseCommissionByamt[21 + 1];  //保值平今手续费按金额 HEDGE_CLOSE_COMMISSION_BYAMT    CMONEY     9422
  char     szArbitrageOpenCommissionByqty[21 + 1];//套利开平手续费按手数 ARBITRAGE_OPEN_COMMISSION_BYQTY CMONEY     9423
  char     szArbitrageOpenCommissionByamt[21 + 1];//套利开平手续费按金额 ARBITRAGE_OPEN_COMMISSION_BYAMT CMONEY     9424
  char     szArbitrageCloseCommissionByqty[21 + 1];//套利平今手续费按手数 ARBITRAGE_CLOSE_COMMISSION_BYQTYCMONEY     9425
  char     szArbitrageCloseCommissionByamt[21 + 1];//套利平今手续费按金额 ARBITRAGE_CLOSE_COMMISSION_BYAMTCMONEY     9426
...
struct MATRADEAPI CRspFutureMarginField
{
  ...
  char     szProductId[31 + 1];             //品种代码     PRODUCT_ID                 VARCHAR(31)8705
  char     szDeliveryDate[31 + 1];          //交割期      DELIVERY_DATE              VARCHAR(31)9151
  char     szBuketBuyMarginByqty[21 + 1];   //投机买保证金按手数 BUKET_BUY_MARGIN_BYQTY     CMONEY     9429
  char     szBuketBuyMarginByamt[21 + 1];   //投机买保证金按金额 BUKET_BUY_MARGIN_BYAMT     CMONEY     9430
  char     szBuketSellMarginByqty[21 + 1];  //投机卖保证金按手数 BUKET_SELL_MARGIN_BYQTY    CMONEY     9431
  char     szBuketSellMarginByamt[21 + 1];  //投机卖保证金按金额 BUKET_SELL_MARGIN_BYAMT    CMONEY     9432
  char     szHedgeBuyMarginByqty[21 + 1];   //保值买保证金按手数 HEDGE_BUY_MARGIN_BYQTY     CMONEY     9433
  char     szHedgeBuyMarginByamt[21 + 1];   //保值买保证金按金额 HEDGE_BUY_MARGIN_BYAMT     CMONEY     9434
  char     szHedgeSellMarginByqty[21 + 1];  //保值卖保证金按手数 HEDGE_SELL_MARGIN_BYQTY    CMONEY     9435
  char     szHedgeSellMarginByamt[21 + 1];  //保值卖保证金按金额 HEDGE_SELL_MARGIN_BYAMT    CMONEY     9436
  char     szArbitrageBuyMarginByqty[21 + 1];//套利买保证金按手数 ARBITRAGE_BUY_MARGIN_BYQTY CMONEY     9437
  char     szArbitrageBuyMarginByamt[21 + 1];//套利买保证金按金额 ARBITRAGE_BUY_MARGIN_BYAMT CMONEY     9438
  char     szArbitrageSellMarginByqty[21 + 1];//套利卖保证金按手数 ARBITRAGE_SELL_MARGIN_BYQTYCMONEY     9439
  char     szArbitrageSellMarginByamt[21 + 1];//套利卖保证金按金额 ARBITRAGE_SELL_MARGIN_BYAMTCMONEY     9440
  char     szInstrumentId[31 + 1];         //合约编码     INSTRUMENT_ID              VARCHAR(31)48
};
...
{
  int      iTrdDate;               //交易日期 TRD_DATE   SMALLINT   8844
  char     szSystemDate[32 + 1];     //系统日期 SYSTEM_DATEVARCHAR(32)9093
  char     szSystemTime[32 + 1];     //系统时间 SYSTEM_TIMEVARCHAR(32)9092
};
