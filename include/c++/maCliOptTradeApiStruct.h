...
struct MATRADEAPI CReqOptRpcUserLoginField
{
  ...
  char            chChannel;                  // 操作渠道
  char            szSerChannel[8 + 1];        // 服务渠道
  char			  szClientInfo[512 + 1];	  // 终端信息 1，通过mid接入时，值必须和固定入参F_OP_SITE(操作站点)的值保持一致；2，通过其它方式接入时，值可以为空，如果送值，必须和固定入参F_OP_SITE(操作站点)的值保持一致。3，启用看穿式监管，填入动态加返回的终端信息密文
  char			  szAppId[32 + 1];			  // 终端产品编码 注：开启看穿式监控用到，外围厂商向证券\期货公司注册产品ID
  char			  szOptLicense[256 + 1];	  // 期权授权码 注：开启看穿式监控用到，证券\期货公司向外围厂商发放的接入认证码
  char			  szKeyVer[10 + 1];			  // 密钥版本
  char		      szReturnCode[8 + 1];		  // 异常标识 0-正常 1-终端信息采集为空 2-终端信息加密密钥版本异常 3-终端信息数据异常
  char			  szRelayAppId[32 + 1];		  // 中继产品ID 中继代理软件填写，无中继代理填空
  char			  szClientIp[40 + 1];		  // 客户交易终端公网IP 中继代理软件填写，无中继代理填空
  int			  iClientPort;				  // 客户交易终端公网端口号 中继代理软件填写，无中继代理填空
  char		      szLoginTime[32 + 1];	      // 客户登入时间 格式：2021-05-24 23:00:00，客户登录中继代理时间，中继代理软件填写，无中继代理填空
};
